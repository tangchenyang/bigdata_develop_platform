...
而也随着微服务的实践越来越成熟，我们不难想到，可以用微服务的思想来解决这类问题，将可复用的业务能力，按TDD（领域驱动设计）的原则对业务进行拆分和重组，形成一个统一的业务中台；再对产生的领域数据进行统一建模，产生的指标数据反哺到业务中去，即可形成一个统一的数据中台。  
...
因此，在中小型企业，业务没有那么复杂，并且传统数仓或数据湖架构能够满足战略目标时，数据中台的建设反而会引入过高的技术/架构复杂度和综合成本。我们则需要根据实际情况，按需选择最适合企业的数据平台架构。
